...
export const emailService = {
  ...
  sendOTP: async (email: string, otp: string, type: 'verification' | '2fa' = 'verification') => {
    try {
      ...
      const currentUrl = window.location.origin + '/api/auth/send-otp';
      ...
      if (response.status === 200 && (contentType.includes("text/html") || text.trim().startsWith("<!DOCTYPE") || text.trim().startsWith("<html"))) {
        return { error: `Server routing error: The request to ${currentUrl} returned an HTML page. This usually means your domain is not correctly pointed to the backend service. If you are using Firebase Hosting, please ensure you have configured rewrites to Cloud Run.` };
      }
